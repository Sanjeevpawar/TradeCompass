...
# -------------------------------------------------
# PHASE 12: CANONICAL PRODUCTION DECISION PATH
# -------------------------------------------------
...
def _production_runtime():
    ...
    if _PRODUCTION_RUNTIME is None:
        _PRODUCTION_RUNTIME = ProductionTradeCompassRuntime()
    return _PRODUCTION_RUNTIME


@app.get("/production/v1")
def production_v1():
    """Canonical Phase 12 read-only TradeCompass decision.

    This endpoint emits one production decision only: BUY_CALL, BUY_PUT or WAIT.
    It never places orders and never calculates quantity/capital allocation.
    """
    return _production_runtime().snapshot()


@app.get("/production/signals")
def production_signals(limit: int = 50):
    return {
        "count": min(max(limit, 1), 200),
        "signals": _production_runtime().recent_signals(min(max(limit, 1), 200)),
    }
